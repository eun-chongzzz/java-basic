...
public class Buyer {
	...
	// buyMango(Seller seller, int mango)를 정의하는데 어떤상인에게, 몇개를 살지이며
	public void buyMango(Seller seller, int mango) {
	// 구매하려는 망고총액가 내 소지금을 넘어서면 "돈이 모자랍니다."라고만 알리고 메서드를 중지시킵니다.
		if(mango*4000 > this.mango) {
			System.out.println("돈이 모자랍니다. 소지금 : " + this.money);
			return; // 코드가 진행되지 않도록 강제중지
		}
	// 판매자의 망고가 내가 구매하려는 망고보다 적은경우도 역시 "망고재고가 부족합니다"라고만 알리고 메서드를 중지합니다.
	...
	// 망고개수를 입력받으면 소지금 개수 * 가격 만큼 차감해주시고 망고개수는 올라갑니다.
		// 내 금액은 차감되고 망고가 늘어나는 로직
		this.money -= mango * 4000;
		this.mango += mango;
	// 이때 seller쪽에도 sellMango()가 호출되도록 처리하며
	...
	public void showBuyer() {
		...
		System.out.println("소지중인 망고개수 : " + this.mango +" 잔여금액 : " + this.money);
		System.out.println("--------------------");
	}
}
